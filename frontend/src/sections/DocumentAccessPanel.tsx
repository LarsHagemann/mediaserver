...
export const DocumentAccessPanel = ({ documentId }: Props) => {
  ...
  const { data: access, isLoading, isError } = api.useGetDocumentAccessQuery(documentId);
  const { data: usersData } = api.useListUsersQuery(undefined, { skip: !isAdmin });
  const [updateAccess] = api.useUpdateDocumentAccessMutation();

  const [selectedUserId, setSelectedUserId] = useState("");

  if (isLoading) return <div className="text-sm text-text-secondary p-2">{t("common.loading")}</div>;
  ...
  const handleRemoveShare = (userId: string) => {
    void updateAccess({
      id: documentId,
      isPublic: access.isPublic,
      sharedWith: access.shares.map((s) => s.userId).filter((id) => id !== userId),
    });
  };
  ...
  const availableToAdd = usersData?.users.filter(
  ...
  return (
    <div className="flex flex-col gap-4 p-3 text-sm w-full overflow-y-auto">
      <div>
        <span className="text-text-secondary text-xs uppercase tracking-wide">{t("document.access.owner")}</span>
        <div className="mt-1 font-medium">{ownerLabel}</div>
      </div>

      <div>
        <span className="text-text-secondary text-xs uppercase tracking-wide">{t("document.access.title")}</span>
        <button
        ...
        >
          ...
          <div>
            <div className="font-medium">
              {access.isPublic ? t("document.access.public") : t("document.access.private")}
            </div>
            <div className="text-xs text-text-secondary">
              {access.isPublic ? t("document.access.publicDescription") : t("document.access.privateDescription")}
            </div>
          </div>
        </button>
      </div>
      ...
      {!access.isPublic && (
        <div>
          <span className="text-text-secondary text-xs uppercase tracking-wide">{t("document.access.sharedWith")}</span>
          <div className="mt-1 flex flex-col gap-1">
            {access.shares.length === 0 && (
              <div className="text-text-secondary italic">{t("document.access.noShares")}</div>
            )}
